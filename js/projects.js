document.addEventListener('DOMContentLoaded', () => {
    // Scroll Animation for Sections - Fixed with delay to ensure DOM is fully loaded
    setTimeout(() => {
        const sections = document.querySelectorAll('section');
        sections.forEach(section => {
            // Initially make all sections visible instead of relying solely on observer
    ...
    // FIXED: Improved particles system
    ...
    function animateParticle(particle) {
        ...
        const speedX = Math.random() * 0.5 + 0.1; // px per second
        const speedY = Math.random() * 0.5 + 0.1; // px per second
        ...
        function moveParticle() {
            ...
            if (posX <= 0 || posX >= window.innerWidth) {
                posX = Math.max(0, Math.min(posX, window.innerWidth));
                directionX *= -1;
            }
            
            if (posY <= 0 || posY >= window.innerHeight) {
                posY = Math.max(0, Math.min(posY, window.innerHeight));
                directionY *= -1;
            }
    ...
    // Project filtering functionality - Fixed initialization
    const projectCards = document.querySelectorAll('.project-card');
    let activeFilter = 'all';
    
    // Create filter buttons dynamically if needed
    function createFilterButtons() {
        // Get all project tags first
        const projectTags = document.querySelectorAll('.project-tag');
        if (projectTags.length === 0) return; // Skip if no tags found
    ...
    // Initialize project filters with slight delay to ensure DOM is ready
    setTimeout(() => {
        createFilterButtons();
    }, 100);
